...
- **Theme is light by default and class-based; standalone iframe pages must re-assert it.** Harmonia is light at `:root`, dark via a `.dark` class. The shell hardcodes light (head bootstrap, localStorage key `dirigible.harmonia.theme`) + a sidebar switcher, and `appShell.js` re-applies the saved theme **after** load — because `harmonia.min.js` auto-adds `.dark` from the OS `prefers-color-scheme` (its own key is `codbex.harmonia.colorMode`). A standalone iframe page (task form, standalone report) that doesn't re-assert the theme renders dark on a dark-mode OS even on a light shell (known follow-up). Settings & Reports are **single** sidebar entries opening landing pages, not one nav item per entity. The intent recipe defaults (`IntentSettings.scaffold`) now point model/form/report at the Harmonia templates; glue stays the neutral client-Java template.
