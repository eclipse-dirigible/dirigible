...
- **SPI: `org.eclipse.dirigible.engine.java.spi.JavaClassConsumer`.** Built-in `HandlerClassConsumer` claims `implements JavaHandler` → publishes to `JavaClassRegistry` → dispatched at `/services/java/<project>/<package-path>/<Class>` and `/public/java/...` by `JavaEndpoint`. `data-store-java` ships `EntityClassConsumer` that claims `@Entity` classes → registers them with `JavaEntityManager`. Future "react to compiled client classes" features should plug into this SPI rather than introduce a second synchronizer.
- **Annotations live in `engine-java`.** `org.eclipse.dirigible.engine.java.annotations.*` mirrors JPA signatures: `@Entity`, `@Table`, `@Id`, `@GeneratedValue` (+ `GenerationType`), `@Column`, `@Transient`, `@CreatedAt`/`@UpdatedAt`/`@CreatedBy`/`@UpdatedBy`, `@Documentation`. The engine module is the natural home because it's on the compile-time classpath of every client `.java`.
