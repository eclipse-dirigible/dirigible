...
- **`/dirigible-start`** → `dirigible.mjs build quick` (`-T 1C install -P quick-build`, **no clean** so runtime data under `./target/dirigible` survives) then `dirigible.mjs start`, then shows a `dirigible.mjs logs --lines 40` snapshot. Pass `clean`/`--clean` (e.g. `/dirigible-start clean`) to build with `clean` and reset the DB/repository.
- **`/dirigible-debug`** → same build as start then `dirigible.mjs start --debug` (JDWP on 8000, `suspend=n`), then the same log snapshot. Also accepts `clean`/`--clean`.
- **`/dirigible-stop`** → `dirigible.mjs stop`.
- **`/dirigible-logs`** → `dirigible.mjs logs` in the **foreground** — snapshot by default; `follow` does a bounded foreground tail. (Background tasks don't stream their output into the conversation, so a backgrounded `tail -f` would never surface — hence foreground + bounded.)
