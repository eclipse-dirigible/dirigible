...
**Component reference:** [`.claude/skills/harmonia/SKILL.md`](.claude/skills/harmonia/SKILL.md) is the full codbex-harmonia directive catalog (all `x-h-*` components incl. the `x-h-select` combobox contract, theming, layout), and [`.claude/skills/alpinejs/`](.claude/skills/alpinejs/) covers the Alpine patterns (routing, page components) — both mirrored from `codbex-athena-app`, the reference app this stack was adopted from. Read the skill before changing Harmonia markup.
