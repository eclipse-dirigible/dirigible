...
An audit pass of the help portal against this codebase landed in [dirigible-io/dirigible-io.github.io#121](https://github.com/dirigible-io/dirigible-io.github.io/pull/121) (open) — it fixes the OAuth section, `DIRIGIBLE_HOME_URL` default, the legacy `/services/v4/` prefix in concept pages and tutorials, the architecture/engines/FAQ rewrites for the post-Tomcat Spring Boot runtime, the java-remote-debugging page, and the Helm chart's documented defaults. If a portal page still looks stale after that PR merges, check whether it was in scope (the PR description lists the explicit items deferred — notably `tutorials/customizations/ide/create-view.md` and the Keycloak/Cognito/Snowflake env-var sections).
