...
- **Snapshot:** if the arguments contain `snapshot`, run
  ...
  **Show both, in this order:**
  1. The command's actual log output **verbatim** in a fenced code block — paste the real log
     lines, do NOT replace them with a description.
  2. **Then** a short summary below them: what happened in the window (key events/activity,
     any WARN/ERROR lines called out, or note that the window was idle if there's nothing).
