...
public class ApiIndexGenerator {
    ...
    /**
     * Generates index.md and index.json based on the markdown API files
     * already generated by the parser.
     *
     * @param rootDir the root directory containing the generated API docs
     *                e.g. Paths.get("docs/api")
    ...
    public static void generateApiIndexes(Path rootDir) throws IOException {
        ...
        grouped.keySet().stream().sorted().forEach(group -> {
            indexMd.append("## ").append(group.toUpperCase()).append("\n\n");
            grouped.get(group).stream()
                    .sorted()
                    .forEach(p -> {
                        String name = p.getFileName().toString().replace(".md", "");
                        indexMd.append("- [").append(name).append("](")
                                .append("./").append(group).append("/").append(name).append(".md").append(")\n");
                    });
            indexMd.append("\n");
        });
        ...
        grouped.keySet().stream().sorted().forEach(group -> {
            ...
            grouped.get(group).stream().sorted().forEach(p -> {
                String name = p.getFileName().toString().replace(".md", "");
        ...
        mapper.writerWithDefaultPrettyPrinter()
                .writeValue(rootDir.resolve("index.json").toFile(), rootArray);
    }
