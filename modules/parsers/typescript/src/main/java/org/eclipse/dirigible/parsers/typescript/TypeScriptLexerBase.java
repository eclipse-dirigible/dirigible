...
public abstract class TypeScriptLexerBase extends Lexer {
     ...
     * `${a ? `${X
     *
     * templateDepth will be 2. This variable is needed to determine if a `}` is a plain CloseBrace, or
     * one that closes an expression inside a template string.
     */
    ...
    /**
     * Return the next token from the character stream and records this last token in case it resides on
     * the default channel. This recorded token is used to determine when the lexer could possibly match
     * a regex literal. Also changes scopeStrictModes stack if tokenize special string 'use strict';
     *
