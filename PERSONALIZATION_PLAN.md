...
| **C** | **Personal Shell** — `/services/web/personal/` (`resources-personal`): the personal counterpart of the shared application shell, aggregating every module's personal perspectives under the single 'personal' group; task-first (the Inbox IS the home page — a personal task is how work arrives); no projections/documents/reports (power surfaces). Gotcha encoded: a perspective whose groupId matches no group is SILENTLY DROPPED by the perspectives service, and a group definer must carry `items: []` to classify as a group | **done** |
