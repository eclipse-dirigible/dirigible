...
if (window !== top) {
  ...
  angular
    ...
    .directive(
      ...
      (
        ...
        link: (scope, element) => {
          ...
          notifications.list = JSON.parse(
            localStorage.getItem(notificationStateKey) || "[]",
          );
          ...
          scope.getLocalTime = (timestamp) =>
            new Date(timestamp).toLocaleDateString(navigator.languages, {
              hour: "numeric",
              minute: "numeric",
              day: "numeric",
              month: "numeric",
            ...
            $http({
              ...
              (result) => {
                ...
                if (open) {
                  scope.$evalAsync(() => {
                    if (!exportButton)
                      exportButton = element[0].querySelector(`#exports`);
                    exportButton.focus();
                    exportButton.click();
                  });
                }
                if (open && !scope.exports.length) {
                  ...
                  for (let i = 0; i < scope.exports.length; i++) {
                    if (
                      scope.exports[i].status === scope.ExportStatus.TRIGGRED
                    ) {
                      to = $timeout(() => {
                        getExports(false);
                      }, 2000);
              ...
              (reject) => {
                dialogHub.showAlert({
                  type: AlertTypes.Error,
                  title: LocaleService.t(
                    "exports.error.fetchTitle",
                    "Could not get export list",
                  ),
                  message: LocaleService.t(
                    "unknownErrorMsg",
                    "Please check the console log for more information.",
    ...
    .directive(
      ...
      (Extensions, shellState, Shell, $location, LocaleService, $window) => ({
        restrict: "E",
        transclude: true,
        replace: true,
        scope: {
        ...
        link: {
          pre: (scope, element) => {
            function hasStatusBar() {
              for (
                let i = 0;
                i < element[0].parentElement.children.length;
                i++
              ) {
                if (
                  element[0].parentElement.children[i].classList.contains(
                    "statusbar",
                  ) ||
                  element[0].parentElement.children[i].tagName === "STATUS-BAR"
                ) {
                  return false;
                }
              }
          ...
          post: (scope) => {
            ...
            scope.activeId = URLParams.has("perspective")
              ? URLParams.get("perspective")
              : localStorage.getItem(selectedPerspectiveKey);
            shellState.registerStateListener((data) => {
            ...
            function setDefaultPerspective() {
              if (scope.config.perspectives.length) {
                const label =
                  getPerspectiveLabel(
                    scope.activeId,
                    scope.config.perspectives,
                  ) ??
                  getPerspectiveLabel(scope.activeId, scope.config.utilities);
                if (label) {
                  ...
                  saveSelectedPerspective(
                    scope.config.perspectives[0].items[0].id,
                  );
                } else {
            ...
            scope.switchPerspective = (
              ...
              shellState.perspective = {
                id: id,
                label: translation
                  ? LocaleService.t(translation.key, translation.options, label)
                  : label,
              };
              if (group) {
                scope.$evalAsync(() => {
            ...
            function getPerspectiveLabel(pId, plist) {
              ...
              for (let i = 0; i < plist.length; i++) {
                if (plist[i].path && plist[i].id === pId) {
                  if (plist[i].translation)
                    label = LocaleService.t(
                      plist[i].translation.key,
                      plist[i].translation["options"],
                      plist[i].label,
                    );
            ...
            function getNavCondensed() {
              ...
              if (navCondensed === null) {
                navCondensed =
                  scope.condensed !== undefined ? scope.condensed : true;
            ...
            const showPerspectiveListener = Shell.onShowPerspective((data) => {
              let label = getPerspectiveLabel(
                data.id,
                scope.config.perspectives,
              );
              if (!label)
                label = getPerspectiveLabel(data.id, scope.config.utilities);
              if (label) {
                scope.$evalAsync(() => {
                  shellState.perspective = {
                    id: data.id,
            ...
            scope.$on("$destroy", () => {
              ...
              if (navCondensedListener)
                Shell.removeMessageListener(navCondensedListener);
            });
          },
        },
    ...
    <bk-vertical-nav class="sidebar bk-shadow--none" condensed="condensed" can-scroll="true">
            ...
            <bk-list aria-label="{{'aria.perspectiveList' | t:'Perspective list'}}">
                ...
                <bk-list-navigation-item ng-if="condensed && navItem.items && navItem.items.length" expandable="true" is-expanded="navItem.expanded">
                    ...
                    <bk-list-navigation-item-popover level="2">
                        <bk-list>
                            <bk-list-navigation-item ng-repeat="navGroupItem in navItem.items track by navGroupItem.id" ng-click="switchPerspective($event, navGroupItem.id, navGroupItem.label, navGroupItem.translation, navItem)" indicated="navGroupItem.id === activeId" id="perspective-{{::navGroupItem.id}}">
                ...
                <bk-list-navigation-item ng-if="!condensed && navItem.items && navItem.items.length" expandable="true" is-expanded="navItem.expanded">
                    ...
                    <bk-list-navigation-item-arrow aria-label="{{'aria.expandPerGrp' | t:'expand perspective group'}}"></bk-list-navigation-item-arrow>
                    <bk-list>
                        <bk-list-navigation-item ng-repeat="navGroupItem in navItem.items track by navGroupItem.id" ng-click="switchPerspective($event, navGroupItem.id, navGroupItem.label, navGroupItem.translation)" indicated="navGroupItem.id === activeId" id="perspective-{{::navGroupItem.id}}">
                            <span bk-list-navigation-item-text>{{navGroupItem.translation.key | t:navGroupItem.translation.options:navGroupItem.label}}</span>
                            <bk-list-navigation-item-indicator></bk-list-navigation-item-indicator>
                        </bk-list-navigation-item>
                    </bk-list>
                ...
                <bk-list-navigation-item ng-repeat-end ng-if="!navItem.items" indicated="navItem.id === activeId" ng-click="switchPerspective($event, navItem.id, navItem.label, navItem.translation)" title="{{navItem.translation.key | t:navItem.translation.options:navItem.label}}" id="perspective-{{::navItem.id}}">
                    <bk-list-navigation-item-icon icon-size="lg" svg-path="{{getIcon(navItem.icon)}}"></bk-list-navigation-item-icon>
                    <span bk-list-navigation-item-text>{{navItem.translation.key | t:navItem.translation.options:navItem.label}}</span>
                    <bk-list-navigation-item-indicator></bk-list-navigation-item-indicator>
                </bk-list-navigation-item>
        ...
        <bk-vertical-nav-utility-section ng-if="config.utilities.length" aria-label="{{'aria.utilityNav' | t:'utility navigation'}}">
            <bk-list>
                <bk-list-navigation-item ng-repeat="navItem in config.utilities track by navItem.id" ng-click="switchPerspective($event, navItem.id, navItem.label, navItem.translation)" title="{{navItem.translation.key | t:navItem.translation.options:navItem.label}}" indicated="navItem.id === activeId" id="u-perspective-{{::navItem.id}}">
                    <bk-list-navigation-item-icon icon-size="lg" svg-path="{{getIcon(navItem.icon)}}"></bk-list-navigation-item-icon>
                    <span bk-list-navigation-item-text>{{navItem.translation.key | t:navItem.translation.options:navItem.label}}</span>
                    <bk-list-navigation-item-indicator></bk-list-navigation-item-indicator>
                </bk-list-navigation-item>
    ...
    .directive("notifications", (notifications) => ({
      ...
      link: (scope) => {
        ...
        scope.$on("$destroy", () =>
          notificationHub.removeMessageListener(onNotificationListener),
        );
      },
    ...
    .directive("statusBar", () => ({
      ...
      link: (scope) => {
        ...
        const busyListener = statusBarHub.onBusy((text) =>
          scope.$evalAsync(() => (scope.busy = text)),
        );
        const messageListener = statusBarHub.onMessage((message) =>
          scope.$evalAsync(() => (scope.message = message)),
        );
        const errorListener = statusBarHub.onError((message) =>
          scope.$evalAsync(() => (scope.error = message)),
        );
        const labelListener = statusBarHub.onLabel((label) =>
          scope.$evalAsync(() => (scope.label = label)),
        );
        scope.$on("$destroy", () => {
