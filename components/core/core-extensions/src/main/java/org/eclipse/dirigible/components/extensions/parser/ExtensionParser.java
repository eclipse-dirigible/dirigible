 ...
 */
package org.eclipse.dirigible.components.extensions.parser;
...
public class ExtensionParser {
    ...
    public static final Map<String, ExtensionMetadata> EXTENSIONS = Collections.synchronizedMap(new HashMap<>());
    public static final Map<String, String> MD5 = Collections.synchronizedMap(new HashMap<>());


    /**
     * Extracts @Extension(name, kind) decorator metadata from the TypeScript AST.
     */
    private static class MetadataExtractorVisitor extends TypeScriptParserBaseVisitor<ExtensionMetadata> {

        private final ExtensionMetadata metadata = new ExtensionMetadata();

        @Override
        public ExtensionMetadata visitProgram(TypeScriptParser.ProgramContext ctx) {
            super.visitProgram(ctx);
            return metadata;
        }

        @Override
        public ExtensionMetadata visitClassDeclaration(TypeScriptParser.ClassDeclarationContext ctx) {
        ...
        private void parseClassDecorator(TypeScriptParser.DecoratorContext ctx) {
            ...
            if (!"Extension".equals(decoratorName)) {
                return;
            }
            ...
            if (ctx.decoratorCallExpression() != null && ctx.decoratorCallExpression()
                ...
                metadata.setTo(extractValue(argText, "to"));
            }
        }
    ...
    /**
     * Parses the given TypeScript source and extracts @Extension metadata.
     *
     * @param location the file path or name
     * @param source the TypeScript source code
     * @return the extracted ExtensionMetadata
     */
    public ExtensionMetadata parse(String location, String source) {
        ...
        if (existing != null && existing.equals(md5)) {
            return EXTENSIONS.get(filename);
        }
        ...
        ExtensionMetadata metadata = visitor.visit(tree);

        EXTENSIONS.put(filename, metadata);
