...
`;
  ...
  async function connect(resourcePath) {
    ...
    const wsUrl = `${proto}://${location.host}/websockets/ide/java-lsp?workspace=${encodeURIComponent(workspace)}&project=${encodeURIComponent(project)}`;
    ...
    _conn.onNotification("textDocument/publishDiagnostics", (params) => {
      const model2 = editor.getModels().find((m2) => m2.uri.toString() === params.uri);
      if (!model2) return;
      editor.setModelMarkers(model2, "java-lsp", params.diagnostics.map((d) => ({
    ...
    _conn.sendNotification("textDocument/didOpen", {
      textDocument: {
        ...
        languageId: "java",
        version: 1,
        text: editor.getModels().find((m2) => m2.uri.toString() === _fileUri)?.getValue() ?? ""
      }
    });
    const model = editor.getModels().find((m2) => m2.uri.toString() === _fileUri);
    if (model) {
      let timer;
