 ...
 */
const projectsView = angular.module('projects', [
	'blimpKit',
	'platformView',
	'platformShortcuts',
	'platformEditors',
...
projectsView.controller(
	...
	(
		...
		const isDebugEnabled = [
			...
			'OPR/',
			'Electron/',
		].some((el) => navigator.userAgent.includes(el));
		...
		const jstreeConfig = {
			core: {
				...
				data: (node, cb) => {
					if (node.id === '#') cb(projects);
						...
						WorkspaceService.list(node.data.path).then(
							(response) => {
								let children;
								if (
							...
							() => {
								StatusBar.showError(
									`There was an error while refreshing the contents of '${node.data.path}'`
								);
				...
				keyboard: {
					// We have to have this in order to disable the default behavior.
					enter: () => {},
					f2: () => {},
				},
			},
			...
			state: { key: `${getBrandingInfo().prefix}.view-projects.state` },
			types: {
				'#': {
		...
		function getAutoReveal() {
			const autoRevealKey = `${
				getBrandingInfo().prefix
			}.settings.general.autoReveal`;
		...
		$scope.keyboardShortcuts = (keySet, event) => {
			...
			switch (keySet) {
				case 'enter':
					const focused = jstreeWidget
						.jstree(true)
						.get_node(document.activeElement);
					...
					break;
				case 'shift+enter':
					const toSelect = jstreeWidget
						.jstree(true)
						.get_node(document.activeElement);
					if (toSelect && !toSelect.state.selected)
						jstreeWidget.jstree(true).select_node(toSelect);
					else jstreeWidget.jstree(true).deselect_node(toSelect);
					break;
				case 'f2':
					nodes = jstreeWidget.jstree(true).get_selected(true);
					if (nodes.length === 1)
						openRenameDialog(angular.copy(nodes[0]));
					break;
				case 'delete':
				case 'meta+backspace':
				...
				case 'ctrl+c':
					jstreeWidget
						.jstree(true)
						.copy(jstreeWidget.jstree(true).get_top_selected(true));
					break;
				case 'ctrl+x':
					jstreeWidget
						.jstree(true)
						.cut(jstreeWidget.jstree(true).get_top_selected(true));
					break;
				case 'ctrl+v':
					nodes = jstreeWidget.jstree(true).get_selected(true);
					if (
						nodes.length === 1 &&
						jstreeWidget.jstree(true).can_paste()
					) {
						if (
							nodes[0].type === 'folder' ||
							nodes[0].type === 'project'
						) {
		...
		jstreeWidget.on('paste.jstree', (_event, pasteObj) => {
			...
			const targetPath = parent.data.path.endsWith('/')
				? parent.data.path
				: parent.data.path + '/';
			let pasteArray = [];
					...
					Layout.getCurrentlyOpenedEditors().then((result) => {
						for (let r = 0; r < response.data.length; r++) {
							for (let f = 0; f < result.length; f++) {
								if (
									result[f].startsWith(response.data[r].from)
								) {
									Workspace.announceFileMoved({
										newPath: result[f].replace(
			...
			function onError(response) {
				...
				if (pasteObj.mode !== 'copy_node' && pasteObj.node.length > 1)
					StatusBar.showError(
						`Unable to move ${pasteObj.node.length} objects.`
					);
				else if (
					pasteObj.mode !== 'copy_node' &&
					pasteObj.node.length === 1
				)
					StatusBar.showError(
						`Unable to move '${pasteObj.node[0].text}'.`
					);
				if (pasteObj.mode === 'copy_node' && pasteObj.node.length > 1)
					StatusBar.showError(
						`Unable to copy ${pasteObj.node.length} objects.`
					);
				else if (
					pasteObj.mode === 'copy_node' &&
					pasteObj.node.length === 1
					...
					StatusBar.showError(
						`Unable to copy '${pasteObj.node[0].text}'.`
					);
			}

			if (pasteObj.mode === 'copy_node') {
				WorkspaceService.copy(pasteArray, targetPath).then(
					onResponse,
					onError
				);
			} else {
				for (let pIndex = 0; pIndex < pasteObj.node.length; pIndex++) {
					// Temp solution
					for (let i = 0; i < parent.children.length; i++) {
						const node = jstreeWidget
							.jstree(true)
							.get_node(parent.children[i]);
						if (
							node.text === pasteObj.node[pIndex].text &&
							node.id !== pasteObj.node[pIndex].id
						) {
							Dialogs.showAlert({
								title: 'Could not move',
								message:
									'The destination contains a file/folder with the same name.',
				...
				WorkspaceService.move(
					pasteArray,
					pasteArray.length === 1
						? targetPath + pasteObj.node[0].text
						: targetPath
				).then(onResponse, onError);
			}
		});

		$(document).bind('dnd_stop.vakata', (_e, data) => {
			//Triggered on drag complete
			const target = jstreeWidget
				.jstree(true)
				.get_node(data.event.target);
		...
		function setMenuTemplateItems(parent, menuItems) {
			...
			for (let i = 0; i < children.length; i++) {
				...
				if (lastIndex !== -1)
					childExtensions[
						children[i].substring(lastIndex + 1)
					] = true;
			}
			for (let i = 0; i < menuTemplates.length; i++) {
				...
				if (
					priorityTemplates &&
					!menuTemplates[i].hasOwnProperty('order')
				) {
					item.separator = true;
		...
		function getChildrenNames(node, type = '') {
			...
			if (type) {
				for (let i = 0; i < root.children.length; i++) {
					let child = jstreeWidget
						.jstree(true)
						.get_node(root.children[i]);
					if (child.type === type) names.push(child.text);
				}
			} else {
				for (let i = 0; i < root.children.length; i++) {
					names.push(
						jstreeWidget.jstree(true).get_text(root.children[i])
					);
				}
			}
			return names;
				...
				if (id) {
					...
					if (node.type === 'project') {
						...
						if (nodes.length <= 1) {
							if (
								isDebugEnabled &&
								['js', 'ts', 'mjs', 'cjs'].includes(fileExt)
							)
								items.push({
									id: 'debug',
									label: 'Debug',
									leftIconPath:
										'/services/web/view-projects/images/debugger.svg',
									separator: true,
								});
							items.splice(1, 0, {
								id: 'openWith',
						...
						if (generateObj && GenerateService.isEnabled()) {
							if (fileExt === 'gen') {
								...
								modelTemplates.length &&
								modelTemplateExtensions.includes(fileExt)
							) {
								let genObj = {
									id: 'generateModel',
									label: 'Generate',
				...
				ContextMenu.showContextMenu({
					...
					if (id) {
						if (id === 'open') {
							...
							TransportService.exportProject(
								$scope.selectedWorkspace,
								contextMenuNodes[0].text
							);
							...
							WorkspaceService.loadContent(
										...
										Dialogs.showFormDialog({
											...
											(form) => {
												if (form) {
													gmodel.templateId =
														form['pgfd1'];
													Dialogs.showBusyDialog(
														'Regenerating from model...'
													);
													generateModel(true);
												}
											},
								...
								(response) => {
									...
									Dialogs.showAlert({
										...
										message:
											'There was an error while loading the file. See the log for more information.',
										type: AlertTypes.Error,
										preformatted: false,
									});
									console.error(response);
							...
							for (let i = 0; i < root.children.length; i++) {
								let name = jstreeWidget
									.jstree(true)
									.get_text(root.children[i]);
								projectNames.push({
									label: name,
									value: name,
								});
							...
							if (id === 'generateGeneric') {
								...
								if (contextMenuNodes[0].type !== 'project') {
									const pnode = getProjectNode(
										contextMenuNodes[0].parents
									);
									project = pnode.text;
									generatePath =
										contextMenuNodes[0].data.path.substring(
											pnode.data.path.length
										);
								...
								Dialogs.showFormDialog({
									...
									(form) => {
										if (form) {
											...
											for (
												let i = 0;
												i < genericTemplates.length;
												i++
											) {
												if (
													genericTemplates[i].id ===
													form['pgfd1']
												) {
													template =
														genericTemplates[i];
											...
											GenerateService.generateFromTemplate(
												...
												(response) => {
													...
													StatusBar.showError(
														`Unable to generate from template '${template.name}'`
													);
												}
											);
										}
								...
								project = pnode.text;
								const templateItems = getModelTemplates(
									getFileExtension(contextMenuNodes[0].text)
								);
								Dialogs.showFormDialog({
									title: 'Generate from template',
									form: {
										...
										pgfi1: {
											...
											value: contextMenuNodes[0].data.path.substring(
												pnode.data.path.length + 1
											),
											submitOnEnter: true,
											required: true,
										},
									...
									(form) => generateFromModelHandler(form),
									(error) =>
										generateFromModelErrorHandler(error)
								);
							}
						} else {
							let name;
							...
							for (let i = 0; i < menuTemplates.length; i++) {
								if (menuTemplates[i].id === id) {
									...
									nameless =
										menuTemplates[i].nameless || false;
									staticName =
										menuTemplates[i].staticName || false;
								}
							}
							if (nameless) {
								createFile(
								...
								let excludedNames = getChildrenNames(
									contextMenuNodes[0].id,
									'file'
								);
								let i = 1;
				...
				if (gmodel.model === '') {
					...
					for (let i = 0; i < modelTemplates.length; i++) {
						if (modelTemplates[i].id === gmodel.templateId) {
							for (
								let j = 0;
								j < modelTemplates[i].parameters.length;
								j++
							) {
								newForm[modelTemplates[i].parameters[j].name] =
									{
										controlType:
											modelTemplates[i].parameters[j]
												.type === 'checkbox'
												? 'checkbox'
												: 'input',
										type: 'text',
										label: modelTemplates[i].parameters[j]
											.label,
										required:
											modelTemplates[i].parameters[j]
												.required ?? true,
								...
								if (
									...
									if (
										...
										newForm[
											modelTemplates[i].parameters[j].name
										].hiddenOn = {
											key: modelTemplates[i].parameters[j]
												.ui.hide.property,
											value: modelTemplates[i].parameters[
												j
											].ui.hide.value,
										};
									}
					...
					if (Object.keys(newForm).length > 0) {
						...
						Dialogs.showFormDialog({
							...
							(form) => generateFromModelHandler(form),
							(error) => generateFromModelErrorHandler(error)
						);
					} else {
						generateModel();
					}
					...
					for (let i = 0; i < keys.length; i++) {
						if (form[keys[i]])
							gmodel.parameters[keys[i]] = form[keys[i]];
					}
					generateModel();
				}
			} else {
			...
			WorkspaceService.listWorkspaceNames().then(
				...
				(response) => {
					...
					Notifications.show({
						...
						description:
							'There was an error while trying to load the workspace list.',
					});
				}
			);
		};
		...
		$scope.reloadWorkspace = (setConfig = false) => {
			...
			WorkspaceService.list($scope.selectedWorkspace).then(
				(response) => {
					$scope.$evalAsync(() => {
						for (
							let i = 0;
							...
							if (
								response.data.projects[i].folders &&
								response.data.projects[i].files
							) {
								project['children'] = processChildren(
									response.data.projects[i].folders.concat(
										response.data.projects[i].files
									)
								);
		...
		$scope.loadTemplates = () => {
			...
			TemplatesService.menuTemplates().then(
				(response) => {
				...
				(response) => {
					for (let i = 0; i < response.data.length; i++) {
						if (response.data[i].hasOwnProperty('extension')) {
							...
							modelTemplateExtensions.push(
								response.data[i].extension
							);
						} else {
		...
		$scope.publishAll = () => {
			...
			PublisherService.publish(`/${$scope.selectedWorkspace}/*`).then(
				() => {
					StatusBar.hideBusy();
					StatusBar.showMessage(
						`Published all projects in '${$scope.selectedWorkspace}'`
					);
					Workspace.announcePublished({
						path: `/${$scope.selectedWorkspace}`,
					});
				},
				(response) => {
					console.error(response);
					StatusBar.hideBusy();
					StatusBar.showError(
						`Unable to publish projects in '${$scope.selectedWorkspace}'`
					);
				}
			);
		};
		...
		function unpublishAll() {
			...
			PublisherService.unpublish(`/${$scope.selectedWorkspace}/*`).then(
				() => {
					StatusBar.showMessage(
						`Unpublished all projects in '${$scope.selectedWorkspace}'`
					);
					StatusBar.hideBusy();
					Workspace.announceUnpublished({
						path: `/${$scope.selectedWorkspace}`,
					});
				},
				(response) => {
					...
					StatusBar.hideBusy();
					StatusBar.showError(
						`Unable to unpublish projects in '${$scope.selectedWorkspace}'`
					);
				}
			);
		}
		...
		function publish(path, fileDescriptor, callback) {
			...
			PublisherService.publish(path).then(
				() => {
					...
					if (fileDescriptor)
						Workspace.announcePublished(fileDescriptor);
				},
				(response) => {
					console.error(response);
					StatusBar.hideBusy();
		...
		function unpublish(path, fileDescriptor, callback) {
			...
			PublisherService.unpublish(path).then(
				() => {
					...
					if (callback) callback();
					if (fileDescriptor)
						Workspace.announceUnpublished(fileDescriptor);
				},
				(response) => {
					console.error(response);
					StatusBar.hideBusy();
		...
		function deleteFileFolder(path, nodeId, type) {
			WorkspaceService.remove(path).then(
				() => {
					...
					if (type === 'file')
						Workspace.announceFileDeleted({
							path: path,
						});
					else
						Workspace.announceFolderDeleted({
							path: path,
						});
		...
		function deleteProject(workspace, project, _nodeId) {
			WorkspaceService.deleteProject(workspace, project).then(
				() => {
					...
					Workspace.announceProjectDeleted({
						project: project,
						workspace: workspace,
					});
				},
			...
			Dialogs.showFormDialog({
				...
				(form) => {
					if (form) {
						...
						WorkspaceService.createProject(
							$scope.selectedWorkspace,
							form['pgfi1']
						).then(
							() => {
								Dialogs.closeBusyDialog();
								StatusBar.showMessage(
									`Created project '${form['pgfi1']}'`
								);
								Workspace.announceWorkspaceChanged({
									workspace: $scope.selectedWorkspace,
									params: {
										publish: {
											path: `/${$scope.selectedWorkspace}/${form['pgfi1']}`,
										},
									},
								});
							},
							(response) => {
								...
								StatusBar.showError(
									`Unable to create project '${form['pgfi1']}'`
								);
							}
						);
					}
				...
				if (form) {
					...
					WorkspaceService.copy(originalPath, duplicatePath).then(
						() => {
							Dialogs.closeBusyDialog();
							Workspace.announceWorkspaceChanged({
								workspace: form['pgfd1'],
								params: {
									publish: {
										path: `/${form['pgfd1']}/${form['pgfi1']}`,
									},
								},
							});
							StatusBar.showMessage(
								`Duplicated '${originalPath}'`
							);
						},
						(response) => {
							console.error(response);
							Dialogs.closeBusyDialog();
							StatusBar.showError(
								`Unable to duplicate '${originalPath}'`
							);
							Dialogs.showAlert({
					...
					if (form) {
						...
						WorkspaceService.createWorkspace(form['pgfi1']).then(
							() => {
								Dialogs.closeBusyDialog();
								StatusBar.showMessage(
									`Created workspace '${form['pgfi1']}'`
								);
								Workspace.announceWorkspaceCreated({
							...
							(response) => {
								...
								StatusBar.showError(
									`Unable to create workspace '${form['pgfi1']}'`
								);
							}
						);
					}
					...
					(buttonId) => {
						if (buttonId === 'yes') {
							WorkspaceService.deleteWorkspace(
								...
								() => {
									Workspace.announceWorkspaceDeleted({
										workspace: $scope.selectedWorkspace,
									});
								},
								(response) => {
									console.error(response);
									StatusBar.showError(
										`Unable to delete workspace '${$scope.selectedWorkspace}'`
									);
								}
							);
						}
		...
		function hideSpinner(spinnerId) {
			jstreeWidget
				.jstree(true)
				.delete_node(jstreeWidget.jstree(true).get_node(spinnerId));
		}
			...
			for (let i = 0; i < children.length; i++) {
				...
				if (children[i].folders && children[i].files) {
					child['children'] = processChildren(
						children[i].folders.concat(children[i].files)
					);
		...
		function getEditorsForType(node) {
			...
			if (
				Object.keys(editorsForContentType).indexOf(
					node.data.contentType
				) > -1
			) {
				for (
					let i = 0;
					i < editorsForContentType[node.data.contentType].length;
					i++
				) {
					if (
						editorsForContentType[node.data.contentType][i].id !==
						Editors.defaultEditor.id
					)
						editors.push({
							id: `openWith-${
								editorsForContentType[node.data.contentType][i]
									.id
							}`,
							label: editorsForContentType[node.data.contentType][
								i
							].label,
						});
				}
		...
		function openSelected(nodes) {
			...
			for (let i = 0; i < nodes.length; i++) {
				if (nodes[i].type === 'file') {
					...
					nodes[i].type === 'folder' ||
					nodes[i].type === 'project'
				) {
					if (nodes[i].state.opened)
						jstreeWidget.jstree(true).close_node(nodes[i]);
		...
		function createFile(parent, name, path, content = '') {
			...
			WorkspaceService.createFile(name, path, content).then(
				(response) => {
					WorkspaceService.getMetadataByUrl(response.data).then(
						(metadata) => {
			...
			Dialogs.showFormDialog({
				...
				form: {
					fdti1: {
						...
						inputRules: {
							excluded: excluded
								? excluded
								: getChildrenNames(newNodeData.parent, 'file'),
							patterns: ['^[^/:]*$'],
						},
				...
				(error) => {
					...
					Dialogs.showAlert({
						...
						message:
							'There was an error while processing the new file data.',
						type: AlertTypes.Error,
						preformatted: false,
					});
				}
			...
			Dialogs.showFormDialog({
				...
				(error) => {
					...
					Dialogs.showAlert({
						...
						message:
							'There was an error while processing the new folder data.',
						type: AlertTypes.Error,
						preformatted: false,
					});
				}
			...
			Dialogs.showFormDialog({
				...
				(form) => {
					if (form) {
						WorkspaceService.rename(
							renameNode.text,
							form['fdti1'],
							renameNode.data.path.substring(
								renameNode.data.path.length -
									renameNode.text.length,
								0
							)
						).then(
							(response) => {
								...
								if (renameNode.type === 'file') {
									WorkspaceService.getMetadata(newPath).then(
										(metadata) => {
									...
									Layout.getCurrentlyOpenedEditors(
										renameNode.data.path
									).then((result) => {
										for (
											let i = 0;
											i < result.length;
									...
									for (
										let i = 0;
										i < renameNode.children_d.length;
										i++
									) {
										const child = jstreeWidget
											.jstree(true)
											.get_node(renameNode.children_d[i]);
										child.data.path =
											newPath +
											child.data.path.substring(
												renameNode.data.path.length
											);
									}
				...
				(error) => {
					...
					Dialogs.showAlert({
						title: 'Rename file error',
						message:
							'There was an error while processing the new file name.',
						type: AlertTypes.Error,
						preformatted: false,
					});
				}
			...
			Dialogs.showDialog({
				...
				(error) => {
					...
					Dialogs.showAlert({
						...
						message:
							selected.length > 1
								? `Error while deleting ${selected.length} files.`
								: `Error while deleting '${selected[0].text}'.`,
						type: AlertTypes.Error,
						preformatted: false,
					});
				}
		...
		function generateModel(isRegenerating = false) {
			GenerateService.generateFromModel(
				...
				() => {
					Dialogs.closeBusyDialog();
					StatusBar.showMessage(
						`Generated from model '${gmodel.model}'`
					);
					if (isRegenerating && gmodel.nodeParents.length) {
						jstreeWidget
							.jstree(true)
							.refresh_node(getProjectNode(gmodel.nodeParents));
		...
		Workspace.onFileSaved((fileData) => {
			...
			if (fileData.status) {
				...
				for (let item in instance._model.data) {
					// Uses the unofficial '_model' property but this is A LOT faster then using 'get_json()'
					if (
						item !== '#' &&
						instance._model.data[item].data.path === fileData.path
					) {
						for (
							let i = 0;
							i < instance._model.data[item].parents.length;
							i++
						) {
							if (instance._model.data[item].parents[i] !== '#') {
								if (
									instance._model.data[
										instance._model.data[item].parents[i]
									].type === 'project'
								) {
									if (
										instance._model.data[
											instance._model.data[item].parents[
												i
											]
										].li_attr.git
									) {
										...
										instance.redraw_node(
											instance._model.data[item],
											false,
											false,
											false,
		...
		Workspace.onWorkspaceChanged((changed) => {
			if (changed.workspace === $scope.selectedWorkspace) {
				if (changed.params && changed.params.projectsViewId) {
					jstreeWidget
						.jstree(true)
						.refresh_node(changed.params.projectsViewId);
				} else
					$scope.$evalAsync(() => {
						$scope.reloadWorkspace();
					});
		...
		Workspace.onWorkspaceDeleted((data) => {
			$scope.$evalAsync(() => {
				$scope.workspaceNames.splice(
					$scope.workspaceNames.indexOf(data.workspace),
					1
				);
			...
			if (data.workspace === $scope.selectedWorkspace) {
				...
				if (typeof instance._model !== 'undefined')
					for (let item in instance._model.data) {
						...
						if (
							item !== '#' &&
							instance._model.data[item].type === 'project' &&
							instance._model.data[item].text === data.project
						) {
					...
					if (msg.partial) {
						...
						if (typeof instance._model !== 'undefined') {
							for (let item in instance._model.data) {
								if (
									item !== '#' &&
									instance._model.data[item].data.path ===
										`/${msg.workspace}/${msg.project}`
								) {
