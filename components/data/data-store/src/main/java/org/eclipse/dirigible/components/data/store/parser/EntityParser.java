...
// --- Mock Imports for ANTLR Generated Classes ---
// You must ensure these packages align with your antlr4-maven-plugin configuration
...
public class EntityParser {
	...
	public static class EntityFieldMetadata {
	...
	/**
	 * Custom visitor to traverse the ParseTree and extract entity metadata. The
	 * result of the visit is the EntityMetadata object.
	 */
	private static class MetadataExtractorVisitor extends TypeScriptParserBaseVisitor<EntityMetadata> {
		...
		private String getDecoratorBaseName(TypeScriptParser.DecoratorContext ctx) {
			...
			if (ctx.decoratorCallExpression() != null) {
				...
				memberCtx = ctx.decoratorCallExpression().decoratorMemberExpression();
		...
		public EntityMetadata visitClassDeclaration(TypeScriptParser.ClassDeclarationContext ctx) {
			...
			TerminalNode classNameNode = ctx.identifier().Identifier();
			if (classNameNode != null) {
				currentEntityMetadata.setClassName(classNameNode.getText());
			}

			for (int i = 0; i < ctx.decoratorList().decorator().size(); i++) {
				if (ctx.decoratorList().decorator(i) instanceof TypeScriptParser.DecoratorContext) {
					parseClassDecorator((TypeScriptParser.DecoratorContext) ctx.decoratorList().decorator(i));
				}
			}
		...
		private void parseClassDecorator(TypeScriptParser.DecoratorContext ctx) {
			...
			if ("Entity".equals(decoratorName)) {
				...
				if (ctx.decoratorCallExpression() != null && ctx.decoratorCallExpression().arguments() != null
						&& ctx.decoratorCallExpression().arguments().argumentList().argument().size() > 0) {

					String arg = ctx.decoratorCallExpression().arguments().argumentList().argument(0).getText();
				...
				if (ctx.decoratorCallExpression() != null && ctx.decoratorCallExpression().arguments() != null
					...
					String tableArgText = ctx.decoratorCallExpression().arguments().argumentList().argument(0)
							.getText();

					// Simplified: search for "name:" and extract value (removes quotes)
					if (tableArgText.contains("name:")) {
						String nameValue = tableArgText.substring(tableArgText.indexOf("name:") + 5).trim();
						int end = nameValue.indexOf(",");
						if (end == -1)
							end = nameValue.length() - 1;

						String value = nameValue.substring(0, end).trim().replace("'", "").replace("\"", "");
		...
		/**
		 * Overrides the visit method for class members. We manually iterate over
		 * children to find the DecoratorContexts.
		 */
		@Override
		public EntityMetadata visitClassElement(TypeScriptParser.ClassElementContext ctx) {
			...
			if (propertyDeclaration != null) {
				...
				fieldMetadata.setPropertyName(propertyDeclaration.propertyName().getText());

				if (propertyDeclaration.typeAnnotation() != null) {
					// typeAnnotation includes the colon, type_ is the actual type content.
					fieldMetadata.setTypeScriptType(propertyDeclaration.typeAnnotation().type_().getText());
				} else {
				...
				currentEntityMetadata.getFields().add(fieldMetadata);
			}
		...
		private void parsePropertyDecorator(TypeScriptParser.DecoratorContext ctx, EntityFieldMetadata fieldMetadata,
			...
			if ("Id".equals(decoratorName)) {
				...
				if (ctx.decoratorCallExpression() != null && ctx.decoratorCallExpression().arguments() != null
					...
					String strategy = ctx.decoratorCallExpression().arguments().argumentList().argument(0).getText();
				...
				if (ctx.decoratorCallExpression() != null && ctx.decoratorCallExpression().arguments() != null
					...
					String argText = ctx.decoratorCallExpression().arguments().argumentList().argument(0).getText();
					...
					java.util.function.BiFunction<String, String, String> extractValue = (source, key) -> {
						...
						if (source.contains(pattern)) {
							...
							String valueSegment = source.substring(source.indexOf(pattern) + pattern.length()).trim();
							...
							String value = valueSegment.substring(0, end).trim().replace("{", "").replace("}", "")
									.replace("'", "").replace("\"", "");
							return value;
						}
						return null;
		...
		for (EntityFieldMetadata field : metadata.getFields()) {
			...
			if (field.getColumnDetails() != null) {
				...
				System.out.printf("  Column: %s, Type: %s, Length: %s, Nullable: %s, Default: %s\n",
						...
						cd.getLength() != null ? cd.getLength().toString() : "N/A", cd.isNullable(),
						cd.getDefaultValue());
			}
			System.out.println();
		}
