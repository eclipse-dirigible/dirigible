...
public class DataAsyncExportService {
    ...
    @Autowired
    public DataAsyncExportService(DatabaseExportService databaseExportService, DatabaseMetadataService databaseMetadataService,
    ...
    private void launchExportJob(String datasource, String statement, Optional<JsonElement> parameters, Optional<String> pattern) {
        executorService.submit(() -> {
            ...
            try {
                ...
                String name = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date()) + "-" + getName(statement, pattern) + ".csv";
    ...
    public String getName(String statement, Optional<String> pattern) {
        if (pattern.isPresent())
            return sanitize(pattern.get());
        return sanitize(statement);
    }
