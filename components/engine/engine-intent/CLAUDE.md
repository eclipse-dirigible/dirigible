...
- Process triggers (`trigger: { onCreate: <Entity> }`) fully wired in Java: validated by the parser; the EDM generator adds a `ProcessId` field + a `triggers` collection to the `.model`; the new `template-application-events-java` template generates a `gen/events/<Process>Trigger.java` `@Listener` that starts the process on the entity's create event; the Java DAO template publishes that event. Covered by `IntentEngineIT` (the `triggers` collection in the model + the end-to-end events-template generation through the real generation service). `service-generate/generateUtils.js` gained a `triggers` collection case.
