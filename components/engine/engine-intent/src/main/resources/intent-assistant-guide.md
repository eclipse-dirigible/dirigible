...
- `immutableIn: [<status seed ids>]` (entity-level) - **user-write immutability**: while the entity's
  `function: EntityStatus` relation holds one of the listed seed ids, update and delete through the
  REST surface are rejected with 409 (e.g. `immutableIn: [2]` = a POSTED journal entry is read-only).
  Workflow/system writes through the repository stay possible - corrections to an immutable record
  are reversals generated by the flow, never edits. Requires an EntityStatus relation.
