 ...
 */
package org.eclipse.dirigible.components.websockets.parser;
...
public class WebsocketParser {
    ...
    public static final Map<String, WebsocketMetadata> WEBSOCKETS = Collections.synchronizedMap(new HashMap<>());
    public static final Map<String, String> MD5 = Collections.synchronizedMap(new HashMap<>());


    /**
     * Extracts @Websocket(name, kind) decorator metadata from the TypeScript AST.
     */
    private static class MetadataExtractorVisitor extends TypeScriptParserBaseVisitor<WebsocketMetadata> {

        private final WebsocketMetadata metadata = new WebsocketMetadata();

        @Override
        public WebsocketMetadata visitProgram(TypeScriptParser.ProgramContext ctx) {
            super.visitProgram(ctx);
            return metadata;
        }

        @Override
        public WebsocketMetadata visitClassDeclaration(TypeScriptParser.ClassDeclarationContext ctx) {
        ...
        private void parseClassDecorator(TypeScriptParser.DecoratorContext ctx) {
            ...
            if (!"Websocket".equals(decoratorName)) {
                return;
            }
            ...
            if (ctx.decoratorCallExpression() != null && ctx.decoratorCallExpression()
                ...
                metadata.setEndpoint(extractValue(argText, "endpoint"));
            }
        }
    ...
    /**
     * Parses the given TypeScript source and extracts @Websocket metadata.
     *
     * @param location the file path or name
     * @param source the TypeScript source code
     * @return the extracted WebsocketMetadata
     */
    public WebsocketMetadata parse(String location, String source) {
        ...
        if (existing != null && existing.equals(md5)) {
            return WEBSOCKETS.get(filename);
        }
        ...
        WebsocketMetadata metadata = visitor.visit(tree);

        WEBSOCKETS.put(filename, metadata);
