...
public class DirigibleProcessDiagramCanvas {
    ...
    // Strokes
    protected static final Stroke TASK_BORDER_STROKE = new BasicStroke(3.0f);
    protected static final Stroke THICK_TASK_BORDER_STROKE = new BasicStroke(4.0f);
    protected static final Stroke GATEWAY_TYPE_STROKE = new BasicStroke(4.0f);
    protected static final Stroke EVENT_STROKE = new BasicStroke(3.0f);
    protected static final Stroke END_EVENT_STROKE = new BasicStroke(4.0f);
    protected static final Stroke MULTI_INSTANCE_STROKE = new BasicStroke(3f);
    protected static final Stroke EVENT_SUBPROCESS_STROKE =
            new BasicStroke(2.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 3.0f, new float[] {3.0f}, 0.0f);
    protected static final Stroke NON_INTERRUPTING_EVENT_STROKE =
            new BasicStroke(2.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 3.0f, new float[] {4.0f, 3.0f}, 0.0f);
    protected static final Stroke FLOW_STROKE = new BasicStroke(3f);
    protected static final Stroke HIGHLIGHT_FLOW_STROKE = new BasicStroke(4f);
    protected static final Stroke ANNOTATION_STROKE = new BasicStroke(3.0f);
    protected static final Stroke ASSOCIATION_STROKE =
            new BasicStroke(3.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 3.0f, new float[] {2.0f, 2.0f}, 0.0f);
     ...
     *
     * Allows to specify minimal boundaries on the left and upper side of the canvas. This is useful for
     * diagrams that have white space there. Everything beneath these minimum values will be cropped.
     * It's also possible to pass a specific font name and a class loader for the icon images.
     *
     */
    public DirigibleProcessDiagramCanvas(int width, int height, int minX, int minY, String imageType, String activityFontName,
     ...
     *
     * Allows to specify minimal boundaries on the left and upper side of the canvas. This is useful for
     * diagrams that have white space there (eg Signavio). Everything beneath these minimum values will
     * be cropped.
     *
     ...
     *        horizontal scale will be cropped.
     */
    public DirigibleProcessDiagramCanvas(int width, int height, int minX, int minY, String imageType) {
        this.canvasWidth = width;
        this.canvasHeight = height;
        this.minX = minX;
        this.minY = minY;
    ...
    public void initialize(String imageType) {
        ...
        if (!"png".equalsIgnoreCase(imageType)) {
            this.g.setBackground(new Color(255, 255, 255, 100));
    ...
    public void drawErrorEndEvent(GraphicInfo graphicInfo, double scaleFactor) {
        ...
        g.drawImage(ERROR_THROW_IMAGE, (int) (graphicInfo.getX() + (graphicInfo.getWidth() / 4)),
                (int) (graphicInfo.getY() + (graphicInfo.getHeight() / 4)), (int) (ERROR_THROW_IMAGE.getWidth() / scaleFactor),
    ...
    public void drawEscalationEndEvent(GraphicInfo graphicInfo, double scaleFactor) {
        ...
        g.drawImage(ESCALATION_THROW_IMAGE, (int) (graphicInfo.getX() + (graphicInfo.getWidth() / 4)),
                (int) (graphicInfo.getY() + (graphicInfo.getHeight() / 4)), (int) (ESCALATION_THROW_IMAGE.getWidth() / scaleFactor),
    ...
    public void drawErrorStartEvent(GraphicInfo graphicInfo, double scaleFactor) {
        ...
        g.drawImage(ERROR_CATCH_IMAGE, (int) (graphicInfo.getX() + (graphicInfo.getWidth() / 4)),
                (int) (graphicInfo.getY() + (graphicInfo.getHeight() / 4)), (int) (ERROR_CATCH_IMAGE.getWidth() / scaleFactor),
    ...
    public void drawEscalationStartEvent(GraphicInfo graphicInfo, double scaleFactor) {
        ...
        g.drawImage(ESCALATION_CATCH_IMAGE, (int) (graphicInfo.getX() + (graphicInfo.getWidth() / 4)),
                (int) (graphicInfo.getY() + (graphicInfo.getHeight() / 4)), (int) (ESCALATION_CATCH_IMAGE.getWidth() / scaleFactor),
    ...
    public void drawConditionalStartEvent(GraphicInfo graphicInfo, double scaleFactor) {
        ...
        g.drawImage(CONDITIONAL_CATCH_IMAGE, (int) (graphicInfo.getX() + (graphicInfo.getWidth() / 4)),
                (int) (graphicInfo.getY() + (graphicInfo.getHeight() / 4)), (int) (CONDITIONAL_CATCH_IMAGE.getWidth() / scaleFactor),
                (int) (CONDITIONAL_CATCH_IMAGE.getHeight() / scaleFactor), null);
    }

    public void drawCatchingEvent(GraphicInfo graphicInfo, boolean isInterrupting, BufferedImage image, String eventType,
            double scaleFactor) {

        // event circles
        Ellipse2D outerCircle =
                new Ellipse2D.Double(graphicInfo.getX(), graphicInfo.getY(), graphicInfo.getWidth(), graphicInfo.getHeight());
    ...
    public void drawConnection(int[] xPoints, int[] yPoints, boolean conditional, boolean isDefault, String connectionType,
        ...
        if (associationDirection == AssociationDirection.ONE || associationDirection == AssociationDirection.BOTH) {
            Line2D.Double line = new Line2D.Double(xPoints[xPoints.length - 2], yPoints[xPoints.length - 2], xPoints[xPoints.length - 1],
    ...
    public void drawTask(BufferedImage icon, String name, GraphicInfo graphicInfo, double scaleFactor) {
        ...
        g.drawImage(icon, (int) (graphicInfo.getX() + ICON_PADDING / scaleFactor), (int) (graphicInfo.getY() + ICON_PADDING / scaleFactor),
    ...
    protected void drawMultilineText(String text, int x, int y, int boxWidth, int boxHeight, boolean centered) {
        ...
        while (measurer.getPosition() < characterIterator.getEndIndex() && currentHeight <= boxHeight) {
            ...
            if (currentHeight + height > boxHeight) {
                // The line we're about to add should NOT be added anymore, append three dots to previous one
                // instead
        ...
        for (TextLayout textLayout : layouts) {
            ...
            currentX = x + (centered ? ((boxWidth - ((Double) textLayout.getBounds()
                                                                        .getWidth()).floatValue())
                    / 2) : 0);
    ...
    public void drawExpandedSubProcess(String name, GraphicInfo graphicInfo, boolean isTriggeredByEvent, double scaleFactor) {
        RoundRectangle2D rect =
                new RoundRectangle2D.Double(graphicInfo.getX(), graphicInfo.getY(), graphicInfo.getWidth(), graphicInfo.getHeight(), 8, 8);
    ...
    public void drawExpandedTransaction(String name, GraphicInfo graphicInfo, double scaleFactor) {
        RoundRectangle2D rect =
                new RoundRectangle2D.Double(graphicInfo.getX(), graphicInfo.getY(), graphicInfo.getWidth(), graphicInfo.getHeight(), 8, 8);
        RoundRectangle2D outerRect = new RoundRectangle2D.Double(graphicInfo.getX() - 3, graphicInfo.getY() - 3, graphicInfo.getWidth() + 6,
    ...
    public void drawExclusiveGateway(GraphicInfo graphicInfo, double scaleFactor) {
        ...
        if (scaleFactor == 1.0) {
            ...
            g.setStroke(GATEWAY_TYPE_STROKE);
            Line2D.Double line =
                    new Line2D.Double(x + quarterWidth + 3, y + quarterHeight + 3, x + 3 * quarterWidth - 3, y + 3 * quarterHeight - 3);
            g.draw(line);
     ...
     *
     */
    public List<GraphicInfo> connectionPerfectionizer(SHAPE_TYPE sourceShapeType, SHAPE_TYPE targetShapeType, GraphicInfo sourceGraphicInfo,
            GraphicInfo targetGraphicInfo, List<GraphicInfo> graphicInfoList) {
        ...
        if (graphicInfoList != null && graphicInfoList.size() > 0) {
            ...
            if (shapeFirst != null) {
                graphicInfoFirst.setX(shapeFirst.getBounds2D()
                                                .getCenterX());
                graphicInfoFirst.setY(shapeFirst.getBounds2D()
                                                .getCenterY());
            }
            if (shapeLast != null) {
                graphicInfoLast.setX(shapeLast.getBounds2D()
                                              .getCenterX());
                graphicInfoLast.setY(shapeLast.getBounds2D()
                                              .getCenterY());
            }
            ...
            if (shapeFirst != null) {
                if (graphicInfoList.size() > 1) {
                    Line2D.Double lineFirst = new Line2D.Double(graphicInfoFirst.getX(), graphicInfoFirst.getY(), graphicInfoList.get(1)
                                                                                                                                 .getX(),
                            graphicInfoList.get(1)
                                           .getY());
            ...
            if (shapeLast != null) {
                if (graphicInfoList.size() >= 2) {
                    Line2D.Double lineLast = new Line2D.Double(graphicInfoLast.getX(), graphicInfoLast.getY(),
                            graphicInfoList.get(graphicInfoList.size() - 2)
                                           .getX(),
                            graphicInfoList.get(graphicInfoList.size() - 2)
                                           .getY());
    ...
    private static Point getEllipseIntersection(Shape shape, Line2D.Double line) {
        double angle = Math.atan2(line.y2 - line.y1, line.x2 - line.x1);
        double x = shape.getBounds2D()
                        .getWidth()
                / 2 * Math.cos(angle)
                + shape.getBounds2D()
                       .getCenterX();
        double y = shape.getBounds2D()
                        .getHeight()
                / 2 * Math.sin(angle)
                + shape.getBounds2D()
                       .getCenterY();
