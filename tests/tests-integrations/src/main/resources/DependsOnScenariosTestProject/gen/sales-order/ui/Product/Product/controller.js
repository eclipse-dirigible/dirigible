angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, $http, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('DependsOnScenariosTestProject:defaults.deleteTitle', { name: '$t(DependsOnScenariosTestProject:t.PRODUCT)' });
			translated.deleteConfirm = LocaleService.t('DependsOnScenariosTestProject:messages.deleteConfirm', { name: '$t(DependsOnScenariosTestProject:t.PRODUCT)' });
		});
		$scope.dataPage = 1;
		$scope.dataCount = 0;
		$scope.dataOffset = 0;
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('DependsOnScenariosTestProject:messages.error.unableToLF', { name: '$t(DependsOnScenariosTestProject:t.PRODUCT)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('DependsOnScenariosTestProject:t.PRODUCT'),
					message: LocaleService.t('DependsOnScenariosTestProject:messages.error.unableToCount', { name: '$t(DependsOnScenariosTestProject:t.PRODUCT)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('DependsOnScenariosTestProject:t.PRODUCT'),
							message: LocaleService.t('DependsOnScenariosTestProject:messages.error.unableToDelete', { name: '$t(DependsOnScenariosTestProject:t.PRODUCT)', message: message }),
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
